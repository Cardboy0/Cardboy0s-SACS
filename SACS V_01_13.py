...
anim_last_frame = 250   #Only used if main_anim_copy = True. Tells the script up to which frame of the original anim it should import on the copy. If your scene ends at frame 1000 but you only animated the first 300 you want to set that value to 300, as the other 700 frames are useless and will just take up space and time.

max_baking_time = 0     #in minutes (one second = 0.017 minutes) #Optional - if you give this variable a value higher than 0 the script will either stop once it has baked all frames, or until that amount of time in minutes has passed. It's useful for if you want the script to bake a lot of frames, but don't want to realise 2 hours later when you want to continue that it's only half finished, cause if you abort it yourself things will get messy. Set it to 120 minutes in that case and you can continue at the last baked frame.
...
for x in range(baking_end_f - baking_start_f + 1):
    ...
    print(3*print_symbol_hash + 'current frame:', int((bpy.context.scene.frame_current / time_stretch_multiplier)),'\n'+3*print_symbol_hash )
    prepare_for_vis_or_calc("calc",softbody_dummy,vis_modifiers)
    select_objects([main_anim])
    ...
    if (time.time()-time_script_begin) >= max_baking_time and not max_baking_time == 0:
        print(3*print_symbol_hash + 'Runtime has reached specified limit of ' + str(round((max_baking_time/60),2)) + 'minutes; finishing script early \nlast baked frame:', baking_start_f-1,   '\n'+3*print_symbol_hash )
...
if main_anim_copy == True:
    ...
    select_objects([main_anim])
    export_object_mdd (target_file_ma_copy, "calc", baking_end_f + 1, anim_last_frame, bpy.context.scene.render.fps, False, vis_modifiers)
...
print('script took %s minutes to finish.' % (round(((time_script_end - time_script_begin)/60),2)))
