...
# - Specify some values at the top of the script: desired beginning- and end-frame, your used modifiers of your softbody-dummy, as well as the "time stretch multiplier". I mentioned stretching your keyframes, and that's the value by which they'll get stretched. Only use integers. Concerning the modifiers: Obviously your softbody-dummy needs the softbody-modifier, but you might want to add other modifiers that rely on the shape of an object at the current frame. "Corrective Smooth" is one example, you can't just add it at the end to the finished result, because that result only has one Basis shapekey, but for the modifier to properly work it needs a reference shape of each frame to know what actually is the original shape of that frame.
...
bpy.context.scene.objects[m_a_perm_dupl_calc].name = "result_calc___SB_Script"
bpy.context.scene.objects[m_a_perm_dupl_vis].name  = "result_visual_SB_Script"
